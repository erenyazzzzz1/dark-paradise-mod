...
class Cezalı extends Command {
    ...
    async run(message, args, level) {
        if (!message.member.roles.cache.has("833800729277366332") && !message.member.hasPermission("VIEW_AUDIT_LOG")) return
        ...
        if (member.roles.cache.has("833800729277366332") && !message.member.roles.some(r => ["833800739775578112", "833800739775578112", "833800739775578112", "833800739775578112"])) return this.client.yolla("Yetkililer birbirlerine ceza işlemi uygulayamazlar.", message.author, message.channel)
        ...
        let id = await sunucu.findOne({ guild: "833800057660244018" }).then(res => res.ihlal)
        ...
        if (!message.guild.members.cache.has(user.id)) {
            ...
            await cezalar.findOne({ user: user.id }, async (err, doc) => {
                ...
                member.roles.set(member.roles.cache.has("833800739775578112") ? ["833800739775578112", "833800739775578112"] : ["833800739775578112"]).catch(e => console.log(e))
                ...
                await this.client.channels.cache.get("833986575918563368").send(başe)
                if (!doc) {
                    const newPun = new cezalar({
                        user: user.id,
                        ceza: true,
