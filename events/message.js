...
module.exports = class {
  ...
  async run(message) {
    ...
    if (message.channel.id == "833800771341910047") {
    ...
    if (!owners.includes(message.author.id)) {
      ...
      if (blockArr.length == 9) {
        ...
        this.client.channels.cache.get("833986963920388126").send(`**${message.author.tag}** - ${message.author}(\`${message.author.id}\`) komut engeli yedi.Komut kullanım özeti:\n\`\`\`${blockArr.map(x => x.içerik).join("\n")}\nKullandığı komutlar: ${blockArr.map(x => x.komut).join(",")}\nKullandığı kanallar: ${blockArr.map(x => x.kanal).join(",")}\`\`\``)
